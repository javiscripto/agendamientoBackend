...
class ApartmentService {
    ...
    async getApartmentById(aid) {
        try {
            ...
            console.error("Error al encontrar el departamento");
            throw new Error("Error al encontrar el departamento");
        }; 
    }; 
    ...
    async updateApartment(aid, apartmentDataToReplace) {
        try {
            ...
            const updatedApartmentData = { ...existingApartment.toObject(), ...apartmentDataToReplace };

            // Actualizamos el departamento con los nuevos datos
            const updatedApartment = await apartmentModel.findByIdAndUpdate(aid, updatedApartmentData, { new: true });
